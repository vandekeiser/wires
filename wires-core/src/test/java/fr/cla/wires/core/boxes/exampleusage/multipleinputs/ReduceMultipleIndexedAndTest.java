...
import static fr.cla.wires.support.functional.Indexed.index;
 ...
 */
public class ReduceMultipleIndexedAndTest {
    ...
    private void setup(long multiplicity) {
        ins = Stream.generate(() -> Wire.<Indexed<Boolean>>make()).limit(multiplicity).collect(toList());
        out = Wire.make();
        clock = Clock.createTime();
        ReduceMultipleIndexedAnd.ins(ins).out(out).time(clock);
    }
    ...
    public void out_should_be_false_when_an_input_is_false() {
        given: {
            ins.forEach(i -> i.setSignal(Signal.of(index(0, true))));
            ins.iterator().next().setSignal(Signal.of(index(0, false)));
        }
        when: {
            clock.tick();
        }
    ...
    public void out_should_be_none_when_any_input_is_none__even_if_all_others_are_false() {
        given: {
            ins.forEach(i -> i.setSignal(Signal.of(index(0, false))));
